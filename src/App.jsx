...
const AuthenticatedLayout = ({ children }) => {
  return (
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark">
      <Header />
      <main className="flex-grow pt-4 px-4 md:px-6">
        {children}
      </main>
      <footer className="mt-auto py-3 px-6 text-center text-sm text-gray-500 dark:text-gray-400">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
          <div>
            &copy; {new Date().getFullYear()} DevClub Dashboard. Todos os direitos reservados.
          </div>
          <div className="mt-2 md:mt-0">
            <img src="/devclub-logo.png" alt="DevClub Logo" className="inline w-6 h-6 opacity-50 dark:hidden" />
            <img src="/devclub-logo-w.png" alt="DevClub Logo" className="inline w-6 h-6 opacity-50 hidden dark:inline" />
          </div>
        </div>
      </footer>
    </div>
