...
class TrafficSheetsService {
  ...
  clearCache() {
    ...
    this.lastChannelFetch = null;
  }

  // ============ Quebra por canal (Facebook × Google) ============
  // Vem da aba gid=1122738960 que tem colunas:
  //   col 14 'Dia Face' | col 15 'Valor Gasto Facebook' | col 16 'Leads Face'
  //   col 17 'Dia Google' | col 18 'Valor Gasto Google' | col 19 'Leads Google'
  // Cada linha contém um dia diferente (denormalizado).
  async fetchChannelData() {
    const now = Date.now();
    if (this.channelCache && this.lastChannelFetch && (now - this.lastChannelFetch) < this.cacheTime) {
      return this.channelCache;
    }
    const url = `https://docs.google.com/spreadsheets/d/${this.spreadsheetId}/export?format=csv&gid=1122738960`;
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Facebook: cols 14,15,16
      const dF = (cols[14] || '').trim();
      if (isIsoDate(dF)) {
        facebook.push({ date: dF, spend: toNum(cols[15]), leads: toNum(cols[16]) });
      }
      // Google: cols 17,18,19
      const dG = (cols[17] || '').trim();
      if (isIsoDate(dG)) {
        google.push({ date: dG, spend: toNum(cols[18]), leads: toNum(cols[19]) });
      }
    }

    this.channelCache = { facebook, google };
    this.lastChannelFetch = now;
    return this.channelCache;
  }

  // Filtra por período (Date objects) e agrega totais por canal.
  async getChannelDataByPeriod(startDate, endDate) {
    const data = await this.fetchChannelData();
    const start = startDate instanceof Date ? startDate : new Date(startDate);
    const end = endDate instanceof Date ? endDate : new Date(endDate);
    // Comparar como ISO YYYY-MM-DD evita problemas de timezone
    const sIso = start.toISOString().slice(0, 10);
    const eIso = end.toISOString().slice(0, 10);

    const aggregate = (rows) => {
      const filtered = rows.filter((r) => r.date >= sIso && r.date <= eIso);
    ...
    return {
      facebook: aggregate(data.facebook),
      google: aggregate(data.google),
    };
  }
}
